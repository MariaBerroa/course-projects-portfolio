...
On my/our honor, Maria Berroa, this
...
class Keyboard:
    ...
    def __str__(self):
        ...
        for letter in self.rows[0]:
            first_row += color_word(self.colors[letter],letter)
            first_row += blank_space
        first_row = first_row.rstrip()
        second_row = " "
        for letter in self.rows[1]:
            second_row += color_word(self.colors [letter],letter)
            second_row += blank_space
        second_row = second_row.rstrip()
        third_row = "   "
        for letter in self.rows[2]:
            third_row += color_word(self.colors[letter],letter)
            third_row += blank_space
...
class WordFamily:
    ...
    def __init__(self, feedback_colors, words):
        ...
        #We have color difficulty, we have the words, and we have the feedback colors
        #Let's get the feedback color for every letter of each word
        for i in range(len(self.feedback_colors)):
            color = self.feedback_colors[i]
        ...
        #word family: all words from word list that match a specific pattern of feedback colors
        #from all the words in word list, compare every letter of this word to every letter of the guessed word
        #if letter in guessed word matches letter on word list = green
        #if letter in guessed word is in word on word list = yellow
        #create n number of patterns that exist between the words in the word list. maybe a loop that sends every green/yellow/grey into
        #a new pattern??? label them pattern 1,2,3...
        #now, match each word from word list to a pattern
    ...
    def __lt__(self, other):
        ...
        """
        #check if other is a WordFamily instance
        ...
        #lexicographical order (feedback_color)
        if self.feedback_colors < other.feedback_colors:
            return True
        elif self.feedback_colors > other.feedback_colors:
            return False
...
def fast_sort(lst):
    ...
    left_half = lst[:mid]
    right_half = lst[mid:]
    left_half_sorted = fast_sort(left_half)
    right_half_sorted = fast_sort(right_half)

    # now that they are sorted, let's merge them 

    sorted_list = []
    i,j = 0,0
    while i < len(left_half_sorted) and j < len(right_half_sorted):
        if left_half_sorted[i] < right_half_sorted[j]:
            sorted_list.append(left_half_sorted[i])
            i = i+1
        else:
            sorted_list.append(right_half_sorted[j])
            j = j+1
    ...
    while j < len(right_half_sorted):
        ...
        j = j+1
    return sorted_list
    #return new list with elements in org list in ascending order (small to big)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    secret_word_dictionary = {letter: secret_word_list.count(letter) for letter in secret_word_list}

    #Check for Green
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
           feedback[i] = CORRECT_COLOR
           secret_word_dictionary[guessed_word[i]] -= 1
    #Check for Yellow
    for i in range(NUM_LETTERS):
        if feedback[i] != CORRECT_COLOR:
            if guessed_word[i] in secret_word_list and secret_word_dictionary[guessed_word[i]] > 0:
                feedback[i] = WRONG_SPOT_COLOR
                secret_word_dictionary[guessed_word[i]] -=1
                #if you substract and end up with 0, it means that there are no more
                #letters of that in the word.
